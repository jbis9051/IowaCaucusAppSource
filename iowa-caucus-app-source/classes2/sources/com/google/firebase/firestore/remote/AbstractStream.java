...
abstract class AbstractStream<ReqT, RespT, CallbackT extends Stream.StreamCallback> implements Stream<CallbackT> {
    ...
    AbstractStream(FirestoreChannel firestoreChannel2, MethodDescriptor<ReqT, RespT> methodDescriptor2, AsyncQueue asyncQueue, AsyncQueue$TimerId asyncQueue$TimerId, AsyncQueue$TimerId asyncQueue$TimerId2, CallbackT callbackt) {
        this.firestoreChannel = firestoreChannel2;
        this.methodDescriptor = methodDescriptor2;
        this.workerQueue = asyncQueue;
        this.idleTimerId = asyncQueue$TimerId2;
        this.listener = callbackt;
        this.idleTimeoutRunnable = new IdleTimeoutRunnable();
        this.backoff = new ExponentialBackoff(asyncQueue, asyncQueue$TimerId, BACKOFF_INITIAL_DELAY_MS, BACKOFF_FACTOR, BACKOFF_MAX_DELAY_MS);
    }
    ...
    private void cancelIdleCheck() {
        AsyncQueue$DelayedTask asyncQueue$DelayedTask = this.idleTimer;
        if (asyncQueue$DelayedTask != null) {
            asyncQueue$DelayedTask.cancel();
            this.idleTimer = null;
        }
    }
}
