...
public class ClientInterceptors {
    ...
    static <WReqT, WRespT> ClientInterceptor wrapClientInterceptor(final ClientInterceptor clientInterceptor, final MethodDescriptor$Marshaller<WReqT> methodDescriptor$Marshaller, final MethodDescriptor$Marshaller<WRespT> methodDescriptor$Marshaller2) {
        return new ClientInterceptor() {
            public <ReqT, RespT> ClientCall<ReqT, RespT> interceptCall(final MethodDescriptor<ReqT, RespT> methodDescriptor, CallOptions callOptions, Channel channel) {
                final ClientCall interceptCall = clientInterceptor.interceptCall(methodDescriptor.toBuilder(MethodDescriptor$Marshaller.this, methodDescriptor$Marshaller2).build(), callOptions, channel);
                    ...
                    public void start(final ClientCall.Listener<RespT> listener, Metadata metadata) {
                        interceptCall.start(new PartialForwardingClientCallListener<WRespT>() {
                            public void onMessage(WRespT wrespt) {
                                listener.onMessage(methodDescriptor.getResponseMarshaller().parse(methodDescriptor$Marshaller2.stream(wrespt)));
                            }
