...
class OkHttpClientTransport implements ConnectionClientTransport, ExceptionHandlingFrameWriter.TransportExceptionHandler {
    ...
    OkHttpClientTransport(String str, Executor executor2, FrameReader frameReader, FrameWriter frameWriter2, OkHttpFrameLogger okHttpFrameLogger, int i, Socket socket2, Supplier<Stopwatch> supplier, @Nullable Runnable runnable, SettableFuture<Void> settableFuture, int i2, int i3, Runnable runnable2, TransportTracer transportTracer2) {
        ...
        this.maxInboundMetadataSize = RCTCameraModule.RCT_CAMERA_ORIENTATION_AUTO;
    ...
    private void startStream(OkHttpClientStream okHttpClientStream) {
        ...
        if (i >= 2147483645) {
            this.nextStreamId = RCTCameraModule.RCT_CAMERA_ORIENTATION_AUTO;
            startGoAway(RCTCameraModule.RCT_CAMERA_ORIENTATION_AUTO, ErrorCode.NO_ERROR, Status.UNAVAILABLE.withDescription("Stream ids exhausted"));
            return;
        }
    ...
    public Runnable start(ManagedClientTransport$Listener managedClientTransport$Listener) {
        this.listener = (ManagedClientTransport$Listener) Preconditions.checkNotNull(managedClientTransport$Listener, "listener");
        ...
        if (isForTest()) {
            ...
            this.serializingExecutor.execute(new Runnable() {
                public void run() {
                    ...
                    synchronized (OkHttpClientTransport.this.lock) {
                        int unused2 = OkHttpClientTransport.this.maxConcurrentStreams = RCTCameraModule.RCT_CAMERA_ORIENTATION_AUTO;
        ...
        try {
            ...
            this.serializingExecutor.execute(new Runnable() {
                public void run() {
                    OkHttpClientTransport.this.executor.execute(OkHttpClientTransport.this.clientFrameHandler);
                    synchronized (OkHttpClientTransport.this.lock) {
                        int unused = OkHttpClientTransport.this.maxConcurrentStreams = RCTCameraModule.RCT_CAMERA_ORIENTATION_AUTO;
