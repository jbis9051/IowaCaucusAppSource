...
public class TransformNode extends Node {
    ...
    private static List<TransformConfig> processTransforms(ReadableArray readableArray) {
        ...
        for (int i = 0; i < readableArray.size(); i++) {
            ...
            if (map.hasKey("nodeID")) {
                ...
                if (map.getType(FirebaseAnalytics.Param.VALUE) == ReadableType.String) {
                    obj = map.getString(FirebaseAnalytics.Param.VALUE);
                } else {
                    obj = Double.valueOf(map.getDouble(FirebaseAnalytics.Param.VALUE));
                }
    ...
    public WritableArray evaluate() {
        ...
        for (TransformConfig next : this.mTransforms) {
            arrayList.add(JavaOnlyMap.of(new Object[]{next.propertyName, next.getValue(this.mNodesManager)}));
        }
