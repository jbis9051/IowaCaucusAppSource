...
public class Transaction {
    ...
    static /* synthetic */ DocumentSnapshot lambda$getAsync$0(Transaction transaction2, Task task) throws Exception {
        if (task.isSuccessful()) {
            ...
            if (list.size() == 1) {
                Document document = (MaybeDocument) list.get(0);
                if (document instanceof Document) {
                    return DocumentSnapshot.fromDocument(transaction2.firestore, document, false, false);
                }
                if (document instanceof NoDocument) {
                    return DocumentSnapshot.fromNoDocument(transaction2.firestore, document.getKey(), false, false);
                }
                throw Assert.fail("BatchGetDocumentsRequest returned unexpected document type: " + document.getClass().getCanonicalName(), new Object[0]);
            }
