...
final class BigendianEncoding {
    ...
    private static char[] buildEncodingArray() {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            cArr[i | CipherStorageKeystoreAESCBC.ENCRYPTION_KEY_SIZE] = ALPHABET.charAt(i & 15);
        }
        return cArr;
    }
    ...
    private static byte decodeByte(char c, char c2) {
        boolean z = true;
        boolean z2 = c2 < ASCII_CHARACTERS && DECODING[c2] != -1;
        Utils.checkArgument(z2, "invalid character " + c2);
        if (c >= ASCII_CHARACTERS || DECODING[c] == -1) {
            z = false;
        }
