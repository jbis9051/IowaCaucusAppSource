...
public abstract class VirtualView extends ReactViewGroup {
    ...
    public Path getClipPath(Canvas canvas, Paint paint) {
        ...
        if (this.mClipPath != null) {
            ClipPathView definedClipPath = getSvgView().getDefinedClipPath(this.mClipPath);
            if (definedClipPath != null) {
                if (definedClipPath.mClipRule == 0) {
                    path = definedClipPath.getPath(canvas, paint);
                } else {
                    path = definedClipPath.getPath(canvas, paint, Region.Op.UNION);
                }
                path.transform(definedClipPath.mMatrix);
                path.transform(definedClipPath.mTransform);
                int i = definedClipPath.mClipRule;
                if (i == 0) {
